...
module Collect
  class OdkController < ApplicationController
    ...
    def set_example(record)
      ...
      example = Example.new()
      example.species = record[:species]
        if record[:coordinates] == nil
          example.coordinates = {"lat"=>"0.0", "lng"=>"0.0"}
        else
          example.coordinates = point_coordinates(record)
        end
        example.metadata = metadata_hash(record)
      example
    end
    ...
    ## metadata_hash helps with saving metadata from xlsform 'metadata' form fields
    def metadata_hash(record)
      {
        start: record[:start],
        end: record[:end],
        today: record[:today],
    ...
    ## point_coordinates helps with gps data coming from xlsform 'coordinates' form fields
    def point_coordinates(record)
      {
